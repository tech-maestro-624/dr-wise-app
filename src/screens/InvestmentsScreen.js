...
const getSubCategoryIcon = (subCategoryName) => {
  ...
  if (name.includes('mutual')) return require('../../assets/Icons/mutualFund.png');
  if (name.includes('fixed')) return require('../../assets/Icons/fixedInv.png');
  if (name.includes('bond')) return require('../../assets/Icons/bondInv.png');
  if (name.includes('gold')) return require('../../assets/Icons/goldInv.png');
  if (name.includes('las')) return require('../../assets/Icons/LASInvestments.png');
  if (name.includes('nps')) return require('../../assets/Icons/npsInsurance.png');
  if (name.includes('trading')) return require('../../assets/Icons/investmentsTrading.png');
  return require('../../assets/Icons/mutualFund.png'); // default
};
...
const InvestmentsScreen = () => {
  ...
  // Fetch investment subcategories on component mount
  useEffect(() => {
    const fetchInvestmentSubCategories = async () => {
      try {
        setLoading(true);
        console.log('Fetching categories...');

        // First, get all categories to find the Investment category
        ...
        // Find the Investment category
        const investmentCategory = categoriesData.find(cat =>
          cat.name?.toLowerCase().includes('investment') ||
          cat.name?.toLowerCase().includes('investments')
        );

        if (!investmentCategory) {
          console.log('Investment category not found');
          setSubCategories([]);
          return;
        }

        console.log('Found Investment category:', investmentCategory);

        // Now fetch subcategories with parent category filter
        console.log('Fetching investment subcategories for category ID:', investmentCategory._id);
        const subCategoriesResponse = await getSubCategories({
          condition: JSON.stringify({ parentCategory: investmentCategory._id })
        });
        ...
        console.log('Parsed investment subcategories:', subCategoriesData);
        setSubCategories(subCategoriesData);
      } catch (error) {
        console.error('Error fetching investment subcategories:', error);
        Toast.show({
          type: 'error',
          text1: 'Error',
          text2: 'Failed to load investment categories. Please try again.',
        });
        setSubCategories([]);
      } finally {
        setLoading(false);
